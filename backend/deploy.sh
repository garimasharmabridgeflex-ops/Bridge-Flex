...
# gen) across all three services — 40 functions total (27 core, incl. the
...
echo "=== Deploying functions-core (27 functions) ==="
services/core/deploy.sh

echo "=== Deploying functions-payments (1 function) ==="
services/payments/deploy.sh

echo "=== Deploying functions-communication (12 functions) ==="
services/communication/deploy.sh

echo
echo "=== Done. 40 functions + rules deployed to $PROJECT_ID ($REGION). ==="
