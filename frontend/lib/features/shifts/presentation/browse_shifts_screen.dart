...
final _nurseryLocationAreaProvider =
    FutureProvider.autoDispose.family<String?, String>((ref, nurseryId) async {
  final profile = await ref.watch(profileRepositoryProvider).fetchPublicProfile(nurseryId);
  return profile?.locationArea;
});

final _ownLocationAreaProvider = FutureProvider.autoDispose<String?>((ref) async {
  final uid = ref.watch(authStateProvider).valueOrNull?.uid;
  if (uid == null) return null;
  final profile = await ref.watch(profileRepositoryProvider).fetchPublicProfile(uid);
  return profile?.locationArea;
});
...
class _BrowseShiftsScreenState extends ConsumerState<BrowseShiftsScreen> {
  ...
  List<Shift> _applyFilters(List<Shift> shifts, String? ownArea) {
    ...
    var filtered = shifts.where((s) {
      // Text search
      if (_query.isNotEmpty && !s.title.toLowerCase().contains(_query)) return false;
      ...
      switch (_dateFilter) {
        ...
        case _DateFilter.thisWeek:
          if (s.startTime.isBefore(todayStart) || s.startTime.isAfter(weekEnd)) {
            return false;
          }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        toolbarHeight: firstName.isNotEmpty ? 64 : kToolbarHeight,
        title: Column(
          ...
          Expanded(
            child: RefreshIndicator(
              ...
              child: shiftsAsync.when(
                ...
                data: (shifts) {
                  final filtered =
                      _applyFilters(shifts, ownAreaAsync.valueOrNull);
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          Wrap(
            ...
            children: [
              FilterChip(
                label: Text(noLocation ? 'Near me (set location first)' : 'Near me'),
                avatar: noLocation
          ...
          Wrap(
            ...
            children: _DateFilter.values.map((f) {
              final label = f == _DateFilter.pickDate && dateFilter == _DateFilter.pickDate && pickedDate != null
            ...
            children: [
              ...
              Text(
                ...
                style: Theme.of(context).textTheme.labelMedium?.copyWith(
                      color: minPayRate > 0 ? AppColors.coral : scheme.onSurfaceVariant,
          ...
          Slider(
            ...
            max: 30,
            divisions: 30,
            label: minPayRate == 0 ? 'Any' : '£${minPayRate.toStringAsFixed(0)}/hr',
...
class _NearMeAwareCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return areaAsync.when(
      ...
      data: (area) => area == ownArea ? _card(context, area) : const SizedBox.shrink(),
    );
  }
  ...
  Widget _card(BuildContext context, String? area) {
    ...
    ).animate().fadeIn(delay: (40 * index).ms, duration: 300.ms).slideY(
          begin: 0.05,
          end: 0,
          curve: Curves.easeOutCubic,
        );
