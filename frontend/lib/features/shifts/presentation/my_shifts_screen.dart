      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            Text(isNursery ? 'Your posted shifts' : 'Your booked shifts'),
          ],
        ),
        actions: const [
        ...
        children: [
          Padding(
            ...
            child: SegmentedButton<_ShiftTab>(
              segments: const [
                ButtonSegment(value: _ShiftTab.active, label: Text('Active'), icon: Icon(Icons.flash_on_rounded, size: 16)),
                ButtonSegment(value: _ShiftTab.completed, label: Text('Completed'), icon: Icon(Icons.check_circle_outline_rounded, size: 16)),
                ButtonSegment(value: _ShiftTab.cancelled, label: Text('Cancelled'), icon: Icon(Icons.cancel_outlined, size: 16)),
              ],
                ...
                data: (allShifts) {
                  final filtered = allShifts.where((s) {
                    ...
                    return switch (_selectedTab) {
                      _ShiftTab.active => s.status != ShiftStatus.cancelled && !isDone,
                      _ShiftTab.completed => s.status != ShiftStatus.cancelled && isDone,
                      _ShiftTab.cancelled => s.status == ShiftStatus.cancelled,
                    };
                  }).toList();
                  ...
                  if (filtered.isEmpty) {
                    ...
                    return ListView(
                      children: [
                        ...
                        EmptyState(
                          icon: isNursery ? Icons.post_add_rounded : Icons.event_note_outlined,
                          title: emptyTitle,
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, i) {
                      ...
                      ).animate().fadeIn(delay: (40 * i).ms, duration: 300.ms).slideY(begin: 0.05, end: 0);
                    },
                  );
                },
              ),
